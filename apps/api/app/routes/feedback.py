...
async def _save_feedback(
    ...
    try:
        ...
        # Compatibility path for production environments that still expose
        # partial/legacy unique index shapes and reject ON CONFLICT.
        has_conflict_shape_mismatch = (
            exc.status_code == 400
            and (
                "42p10" in detail_text
                or "no unique or exclusion constraint matching the on conflict specification"
                in detail_text
            )
        )
        if not has_conflict_shape_mismatch:
            raise
...
async def list_feedback(
    ...
    params = {
        ...
        # Feedback is caregiver-specific: callers should only see their own
        # rating/comment state for a given assistant message.
        "user_id": f"eq.{auth.user_id}",
    }
    if ids:
