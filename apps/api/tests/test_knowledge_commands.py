...
def test_save_to_memory_confirms_pending_inferences() -> None:
    ...
    resp = client.post(
        ...
        json=with_conversation(
            {"message": "We’re following Moms on Call for naps", "child_id": child_id},
    ...
    save_resp = client.post(
        "/api/v1/activities",
        json=with_conversation(
            {"message": "Save to memory", "child_id": child_id},
            conversation_id=conversation_id,
        ),
    )
...
def test_save_to_memory_creates_manual_memory_item_when_no_pending() -> None:
    ...
    response = client.post(
        "/api/v1/activities",
        json=with_conversation({"message": message, "child_id": child_id}, conversation_id=conversation_id),
    )
...
def test_save_to_memory_asks_for_clarification_if_no_context_to_save() -> None:
    ...
    response = client.post(
        "/api/v1/activities",
        json=with_conversation({"message": "Save that", "child_id": child_id}, conversation_id=conversation_id),
    )
    assert response.status_code == 200
